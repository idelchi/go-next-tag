...
BINARY=${GO_NEXT_TAG_BINARY:-"go-next-tag"}
VERSION=${GO_NEXT_TAG_VERSION:-"v0.1"}
OUTPUT_DIR=${GO_NEXT_TAG_OUTPUT_DIR:-"./bin"}
DEBUG=${GO_NEXT_TAG_DEBUG:-0}
DRY_RUN=${GO_NEXT_TAG_DRY_RUN:-0}
ARCH=${GO_NEXT_TAG_ARCH}
OS=${GO_NEXT_TAG_OS}
...
Flags and environment variables:
    ...
    -----------------------------------------------------------------
    -b    GO_NEXT_TAG_BINARY     "go-next-tag"  Binary name to install
    -v    GO_NEXT_TAG_VERSION    "v0.0"         Version to install
    -d    GO_NEXT_TAG_OUTPUT_DIR "./bin"        Output directory
    -o    GO_NEXT_TAG_OS         <detected>     Override operating system
    -a    GO_NEXT_TAG_ARCH       <detected>     Override architecture
    -x    GO_NEXT_TAG_DEBUG                     Enable debug output
    -n    GO_NEXT_TAG_DRY_RUN                   Dry run mode
    -h                                          Show this help message

Flags take precedence over environment variables when both are set.

Example:
    GO_NEXT_TAG_VERSION="v1.0" ./install.sh -o /usr/local/bin

Set \`-a\` or \`GO_NEXT_TAG_ARCH\` to download a specific architecture binary.
